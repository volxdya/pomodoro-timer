...
export function TimerControls({ handleStart, handleStop, isCounting, setIsCounting }) { 
    ...
    return (
        <div className="d-flex justify-content-center">
            {!isCounting ? (
                <>
                    <button onClick={handleStart} className="buttonStart">Старт</button>
                    <button className="buttonStop">Стоп</button>
                </>
            ) : (
                <>
                    <button onClick={handleStop} className="buttonPause">Пауза</button>
                    <button className="buttonStopIsCounting" onClick={() => {
                        obj.time = 25 * 60
                        current.set(obj);
